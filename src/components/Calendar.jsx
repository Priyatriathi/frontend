...
const getCompanyOnDate = (day, month, year) => {
  const formattedDay = `${day < 10 ? `0${day}` : day}/${month < 10 ? `0${month}` : month}/${year}`;
...
export default function Calendar() {
  ...
  return (
    <div className="min-h-screen bg-blue-100 text-gray-900 py-10">
      <div className="container mx-auto px-4">
        <h1 className="text-5xl font-bold text-center mb-10" style={{ color: "#0950A0" }}>
            ...
            <div className="w-full max-w-5xl mx-auto">
              ...
              <div className="grid grid-cols-7 gap-2 bg-white text-gray-900 rounded-lg p-6 shadow-lg">
                ...
                {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                  <div key={day} className="font-bold text-lg text-center text-blue-600">
                    {day}
                  </div>
                ))}

                {/* Days with company highlights */}
                {Array.from({ length: months[currentMonth].days }, (_, dayIndex) => {
                  const day = dayIndex + 1;
                  const company = getCompanyOnDate(day, currentMonth + 1, 2024);
                  return (
                    <div
                      key={dayIndex}
                    ...
                    >
                      <div className="text-gray-900 font-semibold">{day}</div>
                      {company && (
                        <div
